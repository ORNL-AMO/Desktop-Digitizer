...
wpd.DataSeries = (function () {
    return function (dim) {
        ...
        this.isEfficiencySet = false;
        this.efficiencyRawData = [];
        ...
        // this.setEfficiencySetStatus = function(val){
        //     isEfficiencySet = val;
        // }
...
wpd.PlotData = (function () {
    var PlotData = function() {
        ...
        this.getDataSeriesAtIndex = function(index){
            return this.dataSeriesColl[index];
        }
...
wpd.dataSeriesManagement = (function () {
    ...
    function manage() {
        if(!wpd.appData.isAligned()) {
            ...
            if(plotData.getDataSeriesCount() == 1){
    ...
    function addSeriesOnAxesChange() {
        var plotData = wpd.appData.getPlotData(),
            seriesName = 'Dataset ' + nameIndex,
            index = plotData.dataSeriesColl.length;
    ...
    function changeSelectedSeries() {
        ...
        if(document.getElementById('efficiency-check').checked){
            document.getElementById('efficiency-check').checked = false;
        }
        updateApp();
        manage();
    }
    ...
    function showEfficiencyPrompt() {
        if(document.getElementById('efficiency-check').checked){
            wpd.popup.show('efficiencyName');
            document.getElementById('efficiencyNameInput').focus();
            document.getElementById('efficiencyNameInput').value = '';
        }
    }

    function efficiencyDataSet(){
        wpd.popup.close('efficiencyName');
        var inputName = document.getElementById('efficiencyNameInput').value;
        ...
        plotData.dataSeriesColl[index] = new wpd.DataSeries();
        plotData.dataSeriesColl[index].name = inputName;
        plotData.dataSeriesColl[index].isEfficiencySet = true;
        plotData.setActiveDataSeriesIndex(index);
        document.getElementById('efficiency-check').checked = false;
        updateApp();
    ...
    function showSelectDataSetWindow(){
        wpd.popup.show('select-data-series-window');
        selectDatasetWindow();
    }

    function selectDatasetWindow() {
        var select1 = document.getElementById('select-data-series-list'),
            select2 = document.getElementById('select-efficiency-data-series-list'),
            plotData = wpd.appData.getPlotData(),
            size = plotData.dataSeriesColl.length,
            i,
            opt;
        ...
        for(i = 0;i < size;i++){
            if(plotData.dataSeriesColl[i].isEfficiencySet === true){
                opt = document.createElement('option');
                opt.value = i;
                opt.innerHTML = plotData.dataSeriesColl[i].name;
    ...
    function setFirstSelected(){
        selectedDataset1 = Number(document.getElementById('select-data-series-list').value);
    }

    function setSecondSelected(){
        selectedDataset2 = Number(document.getElementById('select-efficiency-data-series-list').value);
    }

    function getFirstSelected(){
        return selectedDataset1;
    }

    function getSecondSelected(){
        return selectedDataset2;
    }

    return {
        manage: manage,
        addSeries: addSeries,
        addSeriesOnAxesChange: addSeriesOnAxesChange,
        deleteSeries: deleteSeries,
        viewData: viewData,
        changeSelectedSeries: changeSelectedSeries,
        editSeriesName: editSeriesName,
        showEfficiencyPrompt: showEfficiencyPrompt,
        efficiencyDataSet: efficiencyDataSet,
        selectDatasetWindow: selectDatasetWindow,
        showSelectDataSetWindow: showSelectDataSetWindow,
        setFirstSelected: setFirstSelected,
        setSecondSelected: setSecondSelected,
        getFirstSelected: getFirstSelected,
        getSecondSelected: getSecondSelected,
        updateApp: updateApp
    };
})();
...
wpd.dataTable = (function () {
    ...
    function setupControls() {
        ...
        // Variable Names
        $variableNames.innerHTML = dataCache.fields.join(', ');  //#this is where variable names are displayed
    ...
    function regression(){
        wpd.popup.close('select-data-series-window');
        var plotData = wpd.appData.getPlotData();
        var currentDataSeriesIndex = plotData.getActiveDataSeriesIndex();
        plotData.setActiveDataSeriesIndex(wpd.dataSeriesManagement.getFirstSelected());
        var nonEffDataset = wpd.plotDataProvider.getData();
        plotData.setActiveDataSeriesIndex(wpd.dataSeriesManagement.getSecondSelected());
        var EffDataset = wpd.plotDataProvider.getData();
        plotData.setActiveDataSeriesIndex(currentDataSeriesIndex);
        wpd.dataSeriesManagement.updateApp();
        wpd.Regression.showGraph(nonEffDataset.rawData, EffDataset.rawData);
    }
...
wpd.graphicsWidget = (function () {
    ...
    function getAllContexts() {
        return {
            mainCtx: mainCtx,
            dataCtx: dataCtx,
            efficiencyCtx: efficiencyCtx,
            drawCtx: drawCtx,
            hoverCtx: hoverCtx,
            topCtx: topCtx,
            oriImageCtx: oriImageCtx,
            oriDataCtx: oriDataCtx,
            oriEfficiencyCtx: oriEfficiencyCtx
        };
    }
    ...
    function resize(cwidth, cheight) {
        ...
        $efficiencyCanvas.height = cheight;
    ...
    function init() {
        ...
        $efficiencyCanvas = document.getElementById('efficiencyCanvas');
        ...
        $oriEfficiencyCanvas = document.createElement('canvas');

        mainCtx = $mainCanvas.getContext('2d');
        dataCtx = $dataCanvas.getContext('2d');
        efficiencyCtx = $efficiencyCanvas.getContext('2d');
        ...
        oriEfficiencyCtx = $oriEfficiencyCanvas.getContext('2d');
    ...
    function loadImage(originalImage, imgPath, wasPDF) {
        ...
        $oriEfficiencyCanvas.width = originalWidth;
        $oriEfficiencyCanvas.height = originalHeight;
        ...
        if(wasPDF) fs.unlinkSync(imgPath); ////////modularize into a function that takes an imgs path
    }

    function loadImageFromSrc(imgSrc, wasPDF) {

        if(wasPDF != true) wasPDF = false;

        var originalImage = document.createElement('img');
        originalImage.onload = function () {
            loadImage(originalImage, imgSrc, wasPDF);
        };
    ...
    function fileLoader(fileInfo) {
        if(fileInfo.type.match("image.*")) {
            ...
            wpd.messagePopup.show('ERROR: Invalid File', 'Please load a valid image file or PDF file. Common image formats such as JPG, PNG, BMP, GIF etc. should work. Word documents are not accepted.');
            wpd.busyNote.close();
        }
    }


    function loadNewFileStart() {
        var imgPath;
        // require('./helpers/appMenu.js'); //load menu
        function loadPDF() {
            var wasPDF = true;
            loadImageFromSrc(imgPath, wasPDF);
        }
        var fileLoadElem = document.getElementById('fileLoadBoxStart');

        if(fileLoadElem.files[0].type.match("application/pdf")){

            imgPath = wpd.pdfConverter.convertToImage(fileLoadElem.files[0]);
            wpd.busyNote.show();
            setTimeout(loadPDF, 1500);  //find an adequate time interval

        }
        if(fileLoadElem.files.length == 1 && !fileLoadElem.files[0].type.match("application/pdf")) {
            var fileInfo = fileLoadElem.files[0];
            wpd.busyNote.show();
            fileLoader(fileInfo);
        }
        imageLoaded = true;
        wpd.popup.close('startDiv');
        wpd.popup.show('axesList');
    }
    ...
    function loadNewFile() {
        ...
        if(fileLoadElem.files[0].type.match("application/pdf")){
            ...
            imgPath = wpd.pdfConverter.convertToImage(fileLoadElem.files[0]);
            wpd.busyNote.show();
            setTimeout(loadPDF, 3000);  //find an adequate time interval
    ...
    return {
        ...
        load: loadNewFile,
        loadNewFileStart: loadNewFileStart,
...
wpd.alignAxes = (function () {
    ...
    function initiatePlotAlignment() {
        ...
        if (xyEl.checked === true) {
            isXY = true;
            document.getElementById('efficiency-holder').style.display = 'block';
            document.getElementById('manual-regression-graph').style.display = 'inline-block';
            calibrator = wpd.xyCalibration;
        } else if(barEl.checked === true) {
            isXY = false;
            document.getElementById('efficiency-holder').style.display = 'none';
            document.getElementById('manual-regression-graph').style.display = 'none';
...
wpd.plotDataProvider = (function() {
    ...
    function storeEfficiencyValue(){
        var plotData = wpd.appData.getPlotData(),
            dataSeries = plotData.getActiveDataSeries(),
        efficiencyNum = Number(document.getElementById('efficiencyInput').value);

        if(isNaN(efficiencyNum) || (efficiencyNum < 0)){
            wpd.popup.show('efficiency');
            document.getElementById('efficiencyInput').focus();
            alert('Please enter a valid positive number.');
            return false;
        }

        dataSeries.efficiencyRawData.push(efficiencyNum);
        wpd.popup.close('efficiency');
    }

    function setTempEfficiencyValues(imagePos){

        var plotData = wpd.appData.getPlotData(),
            axes = plotData.axes,
            ptData;

        ptData = axes.pixelToData(imagePos.x, imagePos.y);

        tempData.push(ptData[0]);
    }

    function getEfficiency2DArray(){
        return wpd.appData.getPlotData().getActiveDataSeries().efficiencyRawData;
    }

    function clearEfficiencyData(){
        wpd.appData.getPlotData().getActiveDataSeries().efficiencyRawData = [];
    }

    function getEfficiencyInput(){
    ...
    function getGeneralAxesData() {
        ...
        var plotData = wpd.appData.getPlotData(),
            ...
            connectivityFieldIndices = [],
            rawData = [],
            efficiencyRawData = dataSeries.efficiencyRawData,
            isFieldSortable = [],
            rowi,
            coli,
            pt,
        ...
        //this is where rawData is filled
        for(rowi = 0; rowi < dataSeries.getCount(); rowi++) {

            pt = dataSeries.getPixel(rowi);

            if(dataSeries.isEfficiencySet){
                ptData = axes.pixelToData(pt.x, pt.y);
                ptData[1] = efficiencyRawData[rowi];
    ...
    return {
        ...
        getData: getData,
        setEfficiencyValues: storeEfficiencyValue,
        setTempEfficiencyValue: setTempEfficiencyValues,
        getEfficiencyData: getEfficiency2DArray,
        clearEfficiencyData: clearEfficiencyData,
        getEfficiencyInput: getEfficiencyInput
    };
})();
...
wpd.acquireData = (function () {
    ...
    function manualEfficiencySelection() {
        var tool = new wpd.ManualSelectionTool();
        wpd.graphicsWidget.setTool(tool);
    }
    ...
    function confirmedClearAll() {
        ...
        wpd.plotDataProvider.clearEfficiencyData();
        wpd.graphicsWidget.removeTool();
        wpd.graphicsWidget.resetData();
        wpd.dataPointCounter.setCount();
        wpd.graphicsWidget.removeRepainter();
        ...
        if(isXY){
            // document.getElementById('manual-regression-graph').style.display = 'none';
            document.getElementById('equation-holder').style.visibility = 'hidden';
            document.getElementById('regression-equation').innerHTML = '';
            document.getElementById('selectRegressionOrder').style.display = 'none';

        }
    }

    function clearAll() {
        if(wpd.appData.getPlotData().getActiveDataSeries().getCount() <= 0 && wpd.plotDataProvider.getEfficiencyData().length <= 0) {
            if(isXY){
                // document.getElementById('manual-regression-graph').style.display = 'none';
                document.getElementById('equation-holder').style.visibility = 'hidden';
                document.getElementById('regression-equation').innerHTML = '';
    ...
    return {
        ...
        manualEfficiencySelection: manualEfficiencySelection,
...
wpd.ManualSelectionTool = (function () {
    var Tool = function () {
        ...
        this.onMouseClick = function (ev, pos, imagePos) {
            ...
            if(activeDataSeries.isEfficiencySet){
                wpd.popup.show('efficiency');
                document.getElementById('efficiencyInput').focus();
                document.getElementById('efficiencyInput').value = '';
            }
            ...
            if(plotData.axes.dataPointsHaveLabels) { // e.g. Bar charts
                ...
                if(activeDataSeries.isEfficiencySet){
                    ...
                    activeDataSeries.addPixel(imagePos.x, imagePos.y, activeDataSeries.isEfficiencySet);
                    wpd.graphicsHelper.drawPoint(imagePos, "rgb(0,0,200)");
                }else{
                    isEfficiency = false;
                    activeDataSeries.addPixel(imagePos.x, imagePos.y, activeDataSeries.isEfficiencySet);
...
wpd.download = (function() {
    ...
    function textFile(data, filename, format) { //change name to generate csv
        var Array2d = data.split("\\n").map(function(e){
        ...
        wpd.writeFile.writeData(Array2d, filename, format);
    }
    ...
    function json(jsonData, filename) {
        ...
        wpd.writeFile.writeData(jsonData, filename, 'json');
    }
...
wpd.saveResume = (function () {
    ...
    function resumeFromJSON(json_data) {
       ...
       for(i = 0; i < rdata.dataSeries.length; i++) {
           ...
           currDataset.isEfficiencySet = ds.isEfficiencySet;
           currDataset.efficiencyRawData = ds.efficiencyRawData;
    ...
    function generateJSON() {
        ...
        for(i = 0; i < plotData.dataSeriesColl.length; i++) {
            ...
            outData.wpd.dataSeries[i] = {
                name: ds.name,
                isEfficiencySet: ds.isEfficiencySet,
                efficiencyRawData: ds.efficiencyRawData,
                data: []
            };
    ...
    function read() {
        var input = dialog.showOpenDialog();
        wpd.popup.close('import-json-window');
        jsonfile.readFile(input[0], function(err, obj) {
            resumeFromJSON(JSON.parse(obj));
            wpd.graphicsWidget.resetData();
            wpd.graphicsWidget.removeTool();
            wpd.graphicsWidget.removeRepainter();
            if(wpd.appData.isAligned()) {
...
ipcRenderer.on('loadNewImage', function(){
    if(!imageLoaded){
        wpd.popup.close('startDiv');
    }
...
ipcRenderer.on('exportJSON', function(){
    wpd.saveResume.save();
});

ipcRenderer.on('importJSON', function(){
    wpd.saveResume.load();
});

ipcRenderer.on('editAlignment', function(){
    wpd.alignAxes.editAlignment();
});

ipcRenderer.on('removeGrid', function(){
    wpd.gridDetection.start();
});

ipcRenderer.on('transformationEquations', function(){
    wpd.transformationEquations.show();
});

ipcRenderer.on('acquireData', function(){
    wpd.acquireData.load();
});

ipcRenderer.on('manageDataSets', function(){
    wpd.dataSeriesManagement.manage();
});

ipcRenderer.on('measureDistance', function(){
    wpd.measurement.start(wpd.measurementModes.distance);
});

ipcRenderer.on('measureAngles', function(){
    wpd.measurement.start(wpd.measurementModes.angle);
});

ipcRenderer.on('zoomIn', function(){
    wpd.graphicsWidget.zoomIn();
});

ipcRenderer.on('zoomOut', function(){
    wpd.graphicsWidget.zoomOut();
});

ipcRenderer.on('actualSize', function(){
    wpd.graphicsWidget.zoom100perc();
});

ipcRenderer.on('fit', function(){
    wpd.graphicsWidget.zoomFit();
});

ipcRenderer.on('extendCrosshair', function(){
    wpd.graphicsWidget.toggleExtendedCrosshairBtn();
});

ipcRenderer.on('crosshairZoomSettings', function(){
    wpd.zoomView.showSettingsWindow();
});
